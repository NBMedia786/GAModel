...
async function processAnalysisJob(videoPath, prompt, res) {
  ...
  try {
    ...
    for (let i = 0; i < totalChunks; i++) {
      ...
      const analysisPrompt = prompt || "Analyze this video for errors in subtitles, grammar, spelling, and punctuation. Provide timestamps and corrections.";
    ...
    // Notify Completion
    const videoName = path.basename(videoPath);
    await addNotification('Analysis Complete', `${videoName} is done with ai analysis`, 'success');
...
// REMOVED OLD ROUTES
// app.post('/upload', ...)
// app.post('/analyze/:projectId/:fileName', ...)
